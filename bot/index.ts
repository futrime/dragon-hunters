...
async function main() {
  ...
  const registry_address = process.env.REGISTRY_ADDRESS ?? undefined;
...
async function registerBot(
  ...
  if (response.status !== 201) {
    throw new Error(
      `failed to fetch ${registry_address}/api/bots:  ${response.status} ${response.statusText}`
    );
  }
  ...
  if (valid !== true) {
    throw new Error(
      `got invalid response fetching ${registry_address}/api/bots: ${ajv.errorsText(
        validate.errors
      )}`
    );
  }
...
async function setupExpress(
  ...
  const app = express()
    ...
    .use(`/api/events`, routerBotsEvents)
    .use((_, res) => {
