...
router.route("/:jobID/:operation").post((async (req, res) => {
  try {
    ...
    if (operation === "start" && job.state !== ActionInstanceState.READY) {
      ...
      errorMessage = `Job ${job.id} cannot be paused because its state is ${job.state}.`;
    } else if (
      operation === "resume" &&
      job.state !== ActionInstanceState.PAUSED
    ) {
      errorMessage = `Job ${job.id} cannot be resumed because its state is ${job.state}.`;
    } else if (
      operation === "cancel" &&
