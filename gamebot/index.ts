...
async function registerBot(
  ...
  const response = await fetch(
    ...
    throw new Error(
      `failed to fetch http://${gateway_host}:${gateway_port}/api/bots: ${error.message}`
    );
  });

  if (response.status !== 201) {
    throw new Error(
      `failed to fetch http://${gateway_host}:${gateway_port}/api/bots:  ${response.status} ${response.statusText}`
    );
  }
  ...
  if (valid !== true) {
    throw new Error(
      `got invalid response fetching http://${gateway_host}:${gateway_port}/api/bots: ${ajv.errorsText(
        validate.errors
      )}`
    );
  }
